...
class boot_kfold :
 ...
 ######################################################################################################################################################################
        # OUTER-LOOP
        IN_BAG_SETS = []; OUT_BAG_SETS = [] # Test set은 이미 만들어짐. 
        BOOTSTRAP_SUPERSETS = []; OOB_SUPERSETS = []  ; # 각각 (kfold->)inbag sample, (inbag->)outbag sample, (train->)test sample을 통해 구한 prediction set(stacking set)
        
        # (train->)test sample part : Stacking Test set 
        print('TEST STACKING')
        train_df_list_temp, _ = add_weight_column(train_df_list=train_df_list, ID_col = ID_col, T_col=T_col, E_col = E_col, boot=False, S=S, window= window)
        TEST_SUPER_SET= level_1_stack(model_specifics_1,ID_col=ID_col, E_col=E_col, T_col = T_col, measure_T_col = measure_T_col, 
                                           window = window, S = S, k_bin = k_bin, 
                                           train_sets=[train_df_list_temp[0],train_df_list_temp[1],train_df_list_temp[2]], validation_sets=test_df_list) 
        
        
        WEIGHT_BAG_SETS = []; OOB_WEIGHT_BAG_SETS = [] ; TEST_WEIGHT_BAG_SETS = [] # 각각 (kfold->)inbag sample, (inbag->)outbag sample, (train->)test sample을 통해 구한 weight
        print('Loop')
        for b in range(B) :
            print('######################################################################')
            ...
            ############################################################################################################
            # INNER-LOOP(k-fold) : (kfold->)inbag sample 에 대응
            ...
            # append stacked dataset(inbag) from bth bootstrapping, b = 1, ... , B
            ## BOOTSTRAP_SUPERSETS : All B (b_TH_STACK) super sets obtained from B bootstrap samples.
            BOOTSTRAP_SUPERSETS.append(b_TH_STACK)
            # Weights to use for 2nd stage model(on loss function)
            WEIGHT_BAG_SETS.append(b_TH_weight)

            ############################################################################################################
            # every b bagging step : (inbag 으로 1st stage model training) -> outbag sample에 prediction (stacking) 
            # b번째 2nd stage model training을 위해
            if boot :
                b_TH_oob_STACK = level_1_stack(model_specifics_1,ID_col=ID_col, E_col=E_col, T_col = T_col, measure_T_col = measure_T_col, window = window, S = S, k_bin = k_bin, 
                                               train_sets=[train_df_list_new[0], train_df_list_new[1], train_df_list_new[2]], 
                                               validation_sets=[train_df_list_oob[0],train_df_list_oob[1],train_df_list_oob[3]])            
                weight_b_OOB = train_df_list_oob[1]['weight']

                OOB_SUPERSETS.append(b_TH_oob_STACK)
                OOB_WEIGHT_BAG_SETS.append(weight_b_OOB)
...
######################################################################################################################################################################                     
        ...
        # store 
        # original inbag oob datasets
        self.inbags = IN_BAG_SETS
        self.outbags = OUT_BAG_SETS
        
        # inbag supersets and weights
        self.train_supersets = BOOTSTRAP_SUPERSETS
        self.weights = WEIGHT_BAG_SETS 

        # oob supersets and weights
        self.oob_supersets = OOB_SUPERSETS
        self.oob_weights = OOB_WEIGHT_BAG_SETS

        
        # test supersets and weight
        self.test_superset = TEST_SUPER_SET
        self.test_weights = TEST_WEIGHT_BAG_SETS # NOT YET!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
