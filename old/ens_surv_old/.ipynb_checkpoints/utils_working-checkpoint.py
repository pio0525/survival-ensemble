...
def add_weight_column(train_df_list, ID_col, T_col, E_col, boot, S, window) :
    ...
    for s in S :
        ...
        cens_prob_oob.append(np.array(KM_cens.predict(train_df_list_outbag[1].loc[train_df_list_outbag[1].LM == s].sort_values(ID_col)[T_col]) + 10**(-10))*n_risk) # add
        
    cens_prob = [item for sublist in cens_prob for item in sublist]
    cens_prob_oob = [item for sublist in cens_prob_oob for item in sublist] # add
     
    IPC_weight_at_b = train_df_list_inbag[1][[ID_col, 'LM',T_col,E_col]].sort_values(['LM',ID_col])
    IPC_weight_at_b_oob = train_df_list_oob[1][[ID_col, 'LM',T_col,E_col]].sort_values(['LM',ID_col]) # add

    IPC_weight_at_b['cens_prob'] = cens_prob; IPC_weight_at_b['weight_IPC'] = 1/IPC_weight_at_b['cens_prob']
    IPC_weight_at_b_oob['cens_prob'] = cens_prob_oob; IPC_weight_at_b_oob['weight_IPC'] = 1/IPC_weight_at_b_oob['cens_prob'] # add
    
    IPC_weight_at_b.loc[((IPC_weight_at_b[T_col] < IPC_weight_at_b['LM']+window)&(IPC_weight_at_b[E_col]==0)),'weight_IPC'] = 0 
    IPC_weight_at_b_oob.loc[((IPC_weight_at_b_oob[T_col] < IPC_weight_at_b_oob['LM']+window)&(IPC_weight_at_b_oob[E_col]==0)),'weight_IPC'] = 0  # add

    IPC_weight_at_b = IPC_weight_at_b[[ID_col, 'LM', 'weight_IPC']]
    IPC_weight_at_b_oob = IPC_weight_at_b_oob[[ID_col, 'LM', 'weight_IPC']]
    ...
    for i in range(1,len(train_df_list_outbag)) : # add
        train_df_list_outbag[i] = train_df_list_outbag[i].merge(IPC_weight_at_b_oob,how='left', on = [ID_col, 'LM'])  # add
        train_df_list_outbag[i]['weight'] = train_df_list_outbag[i]['weight_IPC']*(10**4) + 10**(-10)  # add
        train_df_list_outbag[i] = train_df_list_outbag[i].drop(['weight_boot','weight_IPC'],axis=1)  # add
